...
class DiffusionPipeline(ConfigMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs) -> Self:
        ...
        for kw, arg in init_kwargs.items():
            expected_type = expected_types.get(kw, (inspect.Signature.empty,))
            ...
            # e.g T5Tokenizer but not T5TokenizerFast
                ...
                and not expected_type == (inspect.Signature.empty,)  # Skip if no type annotations
                and not _is_valid_type(arg, expected_type)  # Check type
            ):
                logger.warning(f"Expected types for {kw}: {expected_type}, got {_get_detailed_type(arg)}.")
